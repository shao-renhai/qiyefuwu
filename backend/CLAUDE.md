...
| 流水分析 | **已完成阶段 A** | 客户级多账户合并 + 比率诊断 + 规则引擎报告 |
| 贷款计算器 | 已完成 | 前端页面 |
...
6. ~~征信分析打磨（逾期 Scheme B / 直入录入 / A4 打印）~~ ✅ 已完成（2026-04-17）
7. ~~流水分析阶段 A（多账户合并 + 规则引擎报告）~~ ✅ 已完成（2026-04-17）
8. 流水分析阶段 B：行业系数校准、申贷时限联动、关联方配置 UI
9. 流水分析阶段 C：工资/税费/社保识别、代发名单分布、现金归集度
10. 开发付费报告页面（免费基础版 + 付费完整版分享链路）
11. 建立案例录入界面（供团队录入历史案例，用于校准权重）
12. 融资诊断 manual_data 优先级接入（diagnosis.py 的 _answers_to_input 增加 manual_data 最高优先级）
...
### 2026-04-17（征信分析打磨 + 流水分析阶段 A 上线）

**征信分析打磨**（commits 0136617 / bd8abb1）
- 查询记录简化为仅贷款审批（移除信用卡审批字段）
- 逾期记录改为 Scheme B 卡片：当前逾期 / 连三累六 / 最近逾期距今月数 / 历史逾期次数
  - 存入 `manual_data.overdue_meta`，同时合成 `overdue_records` 列表给分析引擎复用
- 新建客户流程简化：点击"新建客户"直接进入录入 Tab（无需先上传）
  - 后端新增 `POST /api/credit-report/manual-create`（幂等，若已有 manual-entry 则复用）
- A4 打印优化：`@media print` 隐藏侧边栏/按钮/Tab，A4 纵向 14mm 边距

**流水分析阶段 A**（commit 149e87d + 测试修复 8dff203）
- 新模型 `BankAnalysisContext`（客户级补录数据，一对一）
  - 字段：target_loan_amount / existing_monthly_payment / industry
  - 预留阶段 B/C：apply_deadline / related_parties
  - 迁移脚本 `scripts/migrate_bank_context.py`（inspect + create_all 幂等）
- 核心服务 `services/bank_diagnosis.py`（394 行）
  - `merge_client_transactions`：多账户合并 + 跨账户自转识别（对手方命中 client.name / company_name / related_parties 即标记为 duplicate）
  - `compute_ratios`：5 个比率
    - coverage_ratio（月均收入 / 现有月还款，higher_better）
    - balance_ratio（净留存率，higher_better）
    - volatility_coef（月收入变异系数，lower_better）
    - loan_cover_ratio（月均收入 / 目标贷款额，higher_better）
    - low_balance_ratio（最低余额 / 月均收入，higher_better）
  - `build_risks_and_suggestions`：THRESHOLDS 驱动的规则引擎，统一 `_level_for` 做 healthy/medium/high 分级
  - `build_bank_diagnosis_report`：装配完整报告（overview + ratios + risks + suggestions + monthly + top_counterparties）
  - `prefill_monthly_payment_from_credit`：从最新征信 institutions[].monthly_payment 汇总自动预填
- 新增 5 条路由（`routers/bank_statement.py`）
  ```
  GET    /api/bank-statement/client/{id}/statements
  DELETE /api/bank-statement/statement/{id}
  GET    /api/bank-statement/client/{id}/context
  PUT    /api/bank-statement/client/{id}/context
  GET    /api/bank-statement/client/{id}/diagnosis-report
  ```
- 前端 `BankAnalysis.tsx` 完整重构为客户级三 Tab
  - Tab 1 流水文件：列表 + 删除 + 上传（同客户同银行自动替换）
  - Tab 2 补录数据：目标贷款额 / 现有月还款（可自动预填）/ 行业
  - Tab 3 诊断报告：4 张比率卡 + 概览 + 风险预警（high/medium/低信息）+ 月度汇总 + Top5 对手方 + 优化建议 + 打印
  - 客户选择器复用征信分析的 "新建客户直接进入" 模式
- 单测 `tests/test_bank_diagnosis.py`（11 个用例）覆盖：
  - compute_ratios（健康/缺上下文/零收入除零保护）
  - 规则引擎（低覆盖率 → high、健康指标无风险、单一对手方集中、high 排序在 medium 之前）
  - merge_client_transactions（多账户合并、跨账户自转去重、空流水）
  - build_bank_diagnosis_report 端到端 smoke

**阈值配置**（`services/bank_diagnosis.py::THRESHOLDS`）
```python
"coverage":    {"healthy": 2.0,  "warn": 1.5}   # higher_better
"balance":     {"healthy": 0.20, "warn": 0.10}
"volatility":  {"healthy": 0.30, "warn": 0.50}  # lower_better
